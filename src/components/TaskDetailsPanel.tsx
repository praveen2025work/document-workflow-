...
export const TaskDetailsPanel: React.FC<TaskDetailsPanelProps> = ({
  ...
  const [selectedQueryForChat, setSelectedQueryForChat] = useState<any>(null);
    ...
    return (
      <div className="space-y-4">
        ...
        {taskDetails?.queries && taskDetails.queries.length > 0 && (
          <div>
            <Label className="text-sm font-medium">Existing Queries ({taskDetails.queries.length})</Label>
            <div className="mt-2 space-y-2">
              {taskDetails.queries.map((query) => (
          ...
          <Card className="border-primary/20 bg-primary/5">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-2">
                <Button 
                  onClick={handleCreateQueryAndClose} 
        ...
        {(!taskDetails?.queries || taskDetails.queries.length === 0) && !showNewQueryForm && (
          <div className="text-center py-8">
            <MessageSquare className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
            <p className="text-sm text-muted-foreground mb-4">No queries for this task yet.</p>
            <Button 
              onClick={() => setShowNewQueryForm(true)}
              size="sm"
            >
                                  ...
                                  {taskDetails?.queries?.map((query) => (
                                    ...
                                    >
                                      <div className="flex items-start justify-between mb-2">
                                        ...
                                        <Badge 
                                          variant={query.queryStatus === 'OPEN' ? 'destructive' : query.queryStatus === 'RESOLVED' ? 'default' : 'secondary'}
                          ...
                          <div className="w-2/3">
                            <Card className="h-full">
                              <CardHeader className="pb-3">
                                ...
                                <div className="flex items-center gap-2">
                                  <Badge 
                                    variant={selectedQuery.queryStatus === 'OPEN' ? 'destructive' : selectedQuery.queryStatus === 'RESOLVED' ? 'default' : 'secondary'}
                                    className="text-xs"
                                  >
                                    {selectedQuery.queryStatus}
                                  </Badge>
                              ...
                              <CardContent className="flex flex-col h-full">
                                  ...
                                  <div className="space-y-3">
                                    ...
                                    <div className="space-y-2">
                                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                        <User className="h-3 w-3" />
                                        <span>{selectedQuery.raisedByUsername}</span>
                                        <Clock className="h-3 w-3" />
                                ...
                                {selectedQuery.queryStatus === 'OPEN' && canUserComment(selectedQuery) && (
                                  <div className="space-y-3 border-t pt-4">
                                    ...
                                    <div className="flex justify-end gap-2">
                                      <Button
                                        variant="outline"
                                        onClick={() => handleResolveQuery(selectedQuery.queryId)}
                                      >
                                        <CheckCircle className="h-4 w-4 mr-2" />
                                        Mark Resolved
                                      </Button>
