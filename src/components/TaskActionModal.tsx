    ...
    <div className="space-y-4">
      {taskDetails?.reviewFiles && taskDetails.reviewFiles.length > 0 && (
        <div>
          ...
          <div className="mt-2 space-y-2">
            {taskDetails.reviewFiles.map((file) => (
              <Card key={file.instanceFileId}>
                <CardContent className="p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-medium">{file.fileName}</p>
                      <p className="text-sm text-muted-foreground">
                        {formatFileSize(file.fileSize)} • Version {file.version}
                      </p>
                    </div>
                    <Button
                      variant="outline"
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{taskDetails.workflowName}</p>
              </div>
              <div>
                <Label>Status</Label>
              ...
              <div>
                ...
                <p className="font-medium">{taskDetails.assignedToUsername}</p>
              </div>
              <div>
                <Label>Task Type</Label>
                <p className="font-medium">{taskDetails.taskType.replace('_', ' ')}</p>
              </div>
            </div>
            ...
            {taskDetails.taskConfiguration.fileDescription && (
              <div>
                ...
                <p className="text-sm text-muted-foreground">{taskDetails.taskConfiguration.fileDescription}</p>
              </div>
            )}
              ...
              <TabsContent value="actions" className="space-y-4">
                ...
                {taskDetails.taskType === 'FILE_DOWNLOAD' && taskDetails.availableFiles && (
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      {taskDetails.availableFiles.map((file) => (
                        <Card key={file.instanceFileId}>
                          <CardContent className="p-3">
                            <div className="flex items-center justify-between">
                              <div>
                ...
                {taskDetails.instanceTaskFiles && taskDetails.instanceTaskFiles.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {taskDetails.instanceTaskFiles.map((file) => (
                        ...
                        <CardContent className="p-3">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="font-medium">{file.fileName}</p>
                              <p className="text-sm text-muted-foreground">
                                {formatFileSize(file.fileSize)} • Version {file.version} • {file.actionType}
                              </p>
                              {file.fileCommentary && (
                                <p className="text-xs text-muted-foreground mt-1">{file.fileCommentary}</p>
                              )}
                            </div>
                            <Button
                              variant="outline"
